<template>
  <div>
    ...
    <Tooltip v-else content="امضا مدیر" placement="top">
      <Button style="border-style: dashed;width: 80px;height: 80px;"
              :disabled="!$store.getters['auth/canSign'](sectionOrder)"
...
export default {
  ...
  methods : {
    signChecklist() {
      ...
      this.$axios.post(`process/checklist-contract/${this.$route.params.id}`,
        this.formValidate)
        .then(response => {
          this.$store.commit('admin/checklistContract/SET_SECTION_SIGNED' , {user : this.user , section : this.section , status : 'manager'})
          this.$Message.success(response.data.message);
          if(response.data.data != null) {
            this.$emit('setType' , 'confirm')
            // if(response.data.data.order === 6)
  ...
  computed : {
    ...mapState('auth' , ['user']) ,
    signedBy() {
      return  this.$store.getters['admin/checklistContract/getSignedBySection'](this.section.id , 4)
    }
  }
